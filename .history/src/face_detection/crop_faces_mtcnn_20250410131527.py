...
def preprocessing_augmentation_pipeline(input_dir, output_dir, numberOfduplication = 1):
    ...
    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    for i in numberOfduplication:
        for label_folder in input_dir.iterdir():
            (output_dir / label_folder.name).mkdir(exist_oke=True)
            for image_path in label_folder.glob("*.*"):
                img = cv2.imread(str(image_path))
                img = preprocessing(img)
                img = augment_image(img, apply_noise = random.choice([True, False]), apply_blur = ([True, False]),apply_noise_and_blur=random.choice([True, False]))
                image_name = image_path.name + "_aug_" + i
                save_path = output_dir / label_folder.name / image_path.name + i
